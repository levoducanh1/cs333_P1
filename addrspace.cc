// addrspace.cc 
//	Routines to manage address spaces (executing user programs).file
//
...
AddrSpace::InitRegisters()
{
   ...
   // accidentally reference off the end!
    ...
    for (int i = 0; i < MAX_FD_TABLES; i++) {
        for (int j = 0; j < MAX_FDS_PER_TABLE; j++) {
            fd_tables[i][j] = -1; // Set all file descriptors to -1 (unused)
        }
    }
}
